RSpec.describe "Default expected value management" do
  ...
  scenario "provides API endpoints to manage default expected values" do
    ...
    post "/default_expected_values", question_id: q1_id
    expect(response.status).to eq(422)
    expect(error_messages).to include("Value can't be blank")

    post "/default_expected_values", question_id: q2_id, value: "456"
    expect(response.status).to eq(201)
    ...
    get "/default_expected_values", question_id: q1_id
    expect(parsed_json.size).to eq(1)

    get "/default_expected_values/#{id}?#{presentation(question: true)}"
    expect(response.status).to eq(200)
    expect(parsed_json.dig(:question, :id)).to eq(q2_id)
    id = parsed_json.fetch(:id)

    delete "/default_expected_values/#{id}"
    expect(response.status).to eq(200)
    expect(parsed_json.fetch(:id)).to eq(id)

    get "/default_expected_values/#{id}"
    ...
    put "/default_expected_values/#{id}", value: "789"
    expect(response.status).to eq(200)
    expect(parsed_json).to include(value: "789")

    put "/default_expected_values/#{id}", value: " "
    expect(response.status).to eq(422)
